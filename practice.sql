...
SELECT department_name, Round(AVG(salary)) as average_salary FROM employees as e
    INNER JOIN departments as d ON e.department_id = d.department_id
    GROUP BY department_name;

--Count Employees in Each Department
SELECT department_name, COUNT(*) as employee_count FROM employees as e
    INNER JOIN departments as d ON e.department_id = d.department_id
    GROUP BY department_name;
...
SELECT to_char(hire_date, 'YYYY') as hire_year, COUNT(*) as Employee_count FROM employees 
    GROUP BY hire_year;
...
SELECT customer_id, count(*) as "order", SUM(total_amount) as order FROM orders
    GROUP BY customer_id
    HAVING count(*) > 2;


--Find the total amount of orders placed each month in the year 2022.
SELECT to_char(order_date, 'Month') as Month, Count(*) as order FROM orders
